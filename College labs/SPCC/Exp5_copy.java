import java.io.*;
class Exp5 {
    private static final char[][] precedence = { //vec of vec
            { '/', '1' },
            { '*', '1' },
            { '+', '2' },
            { '-', '2' }
    ...
    public static void main(String[] args) throws Exception {
        ...
        char token;
        boolean processed[];
        String[][] operators = new String[10][2]; //vec of pair
        String expr = "", temp;
        ...
        for (i = 0; i < expr.length(); i++) {
            token = expr.charAt(i);
            for (j = 0; j < precedence.length; j++) {//check if char is oper
                if (token == precedence[j][0]) {
                    operators[opc][0] = token + "";//store oper
                    operators[opc][1] = i + "";     // and index in operators array
                    opc++;
                    break;
                }
            }
        ...
        for (i = opc - 1; i >= 0; i--) {
            for (j = 0; j < i; j++) {//bubble sort according to dec ord of precedence
